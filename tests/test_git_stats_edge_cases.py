...
class TestGitStatsEdgeCases:
    ...
    def test_enhanced_commit_message_edge_cases(self):
        ...
        stats = GitStats.get_stats(repo_dir)
        if stats:
            message = GitStats.create_enhanced_commit_message("test", stats, 50)
    ...
    def test_format_file_changes_edge_cases(self):
        ...
        many_changes = [
            {"file": f"file{i}.txt", "added": 5, "deleted": 2, "status": "modified"}
            for i in range(20)
        ]
    ...
    def test_squash_temp_commits_edge_cases(self):
        ...
        result = GitStats.squash_temp_commits(repo_dir, "main", "Test message")
        assert result is False
