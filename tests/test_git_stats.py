...
class TestGitStats:
    """Test GitStats functionality including temp branches and enhanced messages"""
    ...
    def test_squash_temp_commits(self, git_repo):
        ...
        subprocess.run(
            ["git", "commit", "-m", "Temp commit 1"], cwd=git_repo, check=True
        )

        new_file.write_text("Temp change 1\nTemp change 2\n")
        subprocess.run(["git", "add", "."], cwd=git_repo, check=True)
        subprocess.run(
            ["git", "commit", "-m", "Temp commit 2"], cwd=git_repo, check=True
        )

        # Squash into master
        success = GitStats.squash_temp_commits(
            git_repo, "master", "Squashed temp changes"
        )
    ...
    def test_staged_files_handling(self, git_repo):
        ...
        try:
            subprocess.run(
                ["git", "init"], cwd=new_repo, check=True, capture_output=True
            )
            subprocess.run(
                ["git", "config", "user.email", "test@example.com"],
                cwd=new_repo,
            ...
            subprocess.run(
                ["git", "config", "user.name", "Test User"], cwd=new_repo, check=True
            )
